...
function ensureMount(): void {
  ...
  // Drive the light/dark palette from the store. themes.css keys everything off
  // `data-cw-theme` on the shadow host (:host([data-cw-theme='dark'])).
  const host = container;
  effect(() => {
    host.setAttribute('data-cw-theme', widgetState.themeMode);
    const primary = widgetState.themePrimary;
    if (primary) {
      host.style.setProperty('--cw-primary', primary);
      host.style.setProperty('--cw-primary-hover', primary);
    } else {
      host.style.removeProperty('--cw-primary');
      host.style.removeProperty('--cw-primary-hover');
    }
  });
