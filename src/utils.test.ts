...
describe('useLocalTime', () => {
  ...
  it('returns a HH:MM local time string for a resolvable country code', () => {
    const { result } = renderHook(() => useLocalTime('DE' as TCountryCode));
  ...
  it('refreshes the time on the one-second interval', async () => {
    ...
    const { result } = renderHook(() => useLocalTime('GB' as TCountryCode));
  ...
  it('clears the interval on unmount', () => {
    ...
    const { unmount } = renderHook(() => useLocalTime('FR' as TCountryCode));
    unmount();
  ...
  it('returns "-" when the resolved timezone is invalid and Intl throws', () => {
    getTimezonesForCountryMock.mockReturnValue([
      { name: 'Not/A_Real_Timezone' } as Timezone,
    ]);
...
describe('callStatus effect', () => {
  ...
  it('shows "Connecting..." with no duration when connected but recovery is unstable', () => {
    ...
    expect(callStatus.label).toBe('Connecting...');
